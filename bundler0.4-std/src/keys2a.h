...
using namespace std;//remove std:: requirement

using namespace ann_1_1_char;

typedef struct keysInfo {//number of keypoints and length of descriptor vector
	int numKeys;//number of keypoints
	int descrLen;//length of the descriptor vector
} keysInfo;
 ...
 * of keypoints. */
keysInfo ReadKeyFile(int option, const char *filename, unsigned char **keys, 
                keypt_t **info = NULL);
...
keysInfo ReadKeys(int option, FILE *fp, unsigned char **keys, keypt_t **info = NULL);
keysInfo ReadKeysGzip(int option, gzFile fp, unsigned char **keys, keypt_t **info = NULL);
...
std::vector<KeypointMatch> MatchKeys(int num_keys1, int descr_len, unsigned char *k1, 
                                     ANNkd_tree *tree2,
                                     double ratio = 0.6, 
                                     int max_pts_visit = 200);
